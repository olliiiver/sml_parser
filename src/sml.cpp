...
void smlOBISByUnit(long int &val, signed char &scaler, sml_units_t unit)
{
  ...
  while (i < listPos)
  {
    ...
    if (pos == 6)
    {
      ...
      {
        /* 32 bit */
        val = (long int)listBuffer[i + 1] << 24 | (long int)listBuffer[i + 2] << 16 | (long int)listBuffer[i + 3] << 8 | listBuffer[i + 4];
      }
      if (size == 5)
      {
        /* 40 bit */
        val = (long long int)listBuffer[i + 1] << 32 | (long int)listBuffer[i + 2] << 24 | (long int)listBuffer[i + 3] << 16 | (long int)listBuffer[i + 4] << 8 | listBuffer[i + 5];
      }
      if (size == 8)
      {
        /* 56 bit */
        val =
            (long long int)listBuffer[i + 1] << 56 | (long long int)listBuffer[i + 2] << 48 | (long long int)listBuffer[i + 3] << 40 | (long long int)listBuffer[i + 4] << 32 | (long int)listBuffer[i + 5] << 24 | (long int)listBuffer[i + 6] << 16 | (long int)listBuffer[i + 7] << 8 | listBuffer[i + 8];
      }
    }
    i += listBuffer[i] + 1;
  }
